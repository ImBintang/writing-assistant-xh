...
// Cytoscape.js canvas wrapper for knowledge graph visualization (PRD-04 Section 2.4.2)
...
// Category colors for nodes (matching the category badge colors)
...
// Relation type line styles (use cytoscape's typed line-style values)
...
export default function GraphCanvas({ nodes, edges, loading, layout = 'force', onNodeClick }: Props) {
  ...
  const { selectedNode, nodeDetailOpen, selectNode, closeNodeDetail, loadNodeGraph } =
  ...
  useEffect(() => {
    ...
    const elements: cytoscape.ElementDefinition[] = [
      ...
      ...edges.map((e) => ({
        ...
        classes: `relation-${e.relationType}`,
      })),
    ];
    ...
    const cy = cytoscape({
      ...
      style: [
        // Node styles
        {
          ...
          style: {
            ...
            'text-wrap': 'ellipsis',
            width: 'mapData(version, 1, 10, 24, 48)',
            height: 'mapData(version, 1, 10, 24, 48)',
            'border-width': 2,
            'border-color': '#fff',
          },
        },
        ...
        ...Object.entries(RELATION_LINE_STYLES).map(([relType, lineStyle]) => ({
      ...
      layout: LAYOUTS[layout] || DEFAULT_LAYOUT,
      minZoom: 0.1,
      maxZoom: 5,
    ...
    cy.on('tap', 'node', (evt) => {
      ...
      const graphNode: GraphNode = {
        ...
        version: nodeData.version as number,
        attributes: nodeData,
      };
    ...
    // Double click to expand node relations
    cy.on('dbltap', 'node', (evt) => {
      const nodeData = evt.target.data() as Record<string, unknown>;
      loadNodeGraph(nodeData.id as string, 2);
    });
  ...
  }, [nodes, edges, layout]); // Re-initialize when data or layout changes

  // Apply layout change without full re-init
  const applyLayout = useCallback(
    (newLayout: string) => {
      if (!cyRef.current) return;
      const options = LAYOUTS[newLayout] || DEFAULT_LAYOUT;
    ...
    return (
      <div className="flex items-center justify-center h-96 bg-gray-50 rounded-lg">
        <div className="text-center">
          <div className="animate-spin inline-block rounded-full h-8 w-8 border-b-2 border-indigo-600 mb-2" />
    ...
    return (
      <div className="flex items-center justify-center h-96 bg-gray-50 rounded-lg">
        <div className="text-center text-gray-400">
          ...
          <p>暂无图谱数据</p>
          <p className="text-xs mt-1">请先提取知识条目并添加关联关系</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="relative">
      ...
      {/* Zoom controls */}
      <div className="absolute bottom-4 left-4 z-10 flex flex-col gap-1 bg-white rounded-lg shadow-md p-1">
        <button
        ...
        <button
          ...
          className="w-8 h-8 flex items-center justify-center text-gray-600 hover:bg-gray-100 rounded text-xs"
        >
          ⊡
        </button>
      </div>
      ...
      {/* Legend */}
      <div className="absolute bottom-4 right-4 z-10 bg-white rounded-lg shadow-md p-3 text-xs">
        <p className="font-medium text-gray-600 mb-1">图例</p>
        {Object.entries(CATEGORY_COLORS).map(([cat, color]) => (
          <div key={cat} className="flex items-center gap-2 py-0.5">
            <span className="w-3 h-3 rounded-full" style={{ backgroundColor: color }} />
            <span className="text-gray-500">
              {({ characters: '人物', techniques: '功法', locations: '地图', worldbuilding: '世界观', weapons: '武器', alchemy: '丹药', plot: '情节' })[cat]}
            </span>
          </div>
        ))}
      </div>
      ...
      {nodeDetailOpen && selectedNode && (
        <NodeDetail node={selectedNode} onClose={closeNodeDetail} />
      )}
    </div>
  );
}
